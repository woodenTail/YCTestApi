...
from apiframework.common import encry_decry
from apiframework.keywords.keywords_util import get_cases_infos, global_variables, DB
import pytest
...
def run(client,casename):
    ...
    :return:
    '''
    cases_info = get_cases_infos(casename) # {casename:[[接口1的信息],[接口2的信息]]}
    ...
    for step in steps:
        ...
        if extract_data !='': # '{"buyerToken":"$.access_token"\n}'
            ...
            for item in extract_data.items():# item 是(key,value)
                ...
                # 解析jsonpath表达式
                value = str(client.extract_expr(jsonpath_value))
                # 把响应数据里面提取出的变量和值存储到全局变量global_variables
                global_variables[key]=value
        # 6.状态码断言
        expect_status_code = step[5]
        print(expect_status_code)
